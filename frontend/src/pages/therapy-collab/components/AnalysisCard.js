...
import { FiChevronDown, FiActivity, FiMessageSquare, FiVolume2, FiClipboard, FiShield, FiTarget, FiZap } from 'react-icons/fi';
                    ...
                    <div className="mt-6 pt-6 border-t border-white/[0.05] space-y-6 animate-in slide-in-from-top-2 fade-in duration-300">
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                            <div className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-5">
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2 mb-3">
                                    <FiMessageSquare className="text-cyan-400" /> Result Summary
                                </p>
                                <p className="text-sm leading-7 text-slate-300">{resultSummary}</p>
                                {review?.finalSummary && (
                                    <p className="mt-3 text-xs leading-6 text-slate-500">AI baseline: {aiSummary}</p>
                                )}
                            </div>

                            <div className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-5">
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2 mb-3">
                                    <FiTarget className="text-emerald-400" /> Treatment Actions
                                </p>
                                <div className="space-y-3">
                                    {treatmentSuggestions.map((item, index) => (
                                        <div key={`${analysis._id}-treatment-${index}`} className="flex items-start gap-3">
                        ...
                        {review?.reviewedAt && (
                            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
                                <p className="text-[10px] font-bold text-emerald-300 uppercase tracking-[0.2em] flex items-center gap-2 mb-3">
                                    <FiShield className="text-emerald-300" /> Doctor Review
                                </p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-300">
                                    <p>Reviewed by {review.doctorName || review.doctor?.name || 'doctor'}</p>
                                    <p>Reviewed on {format(new Date(review.reviewedAt), 'MMM d, yyyy')}</p>
                                    <p>Status: {getTreatmentStatusLabel(analysis)}</p>
                                    <p>Stage: {getCareStageLabel(analysis)}</p>
                                </div>
                                {review.followUpPlan && (
                                    <p className="mt-4 text-sm leading-7 text-slate-300">{review.followUpPlan}</p>
                                )}
                            </div>
                        )}
