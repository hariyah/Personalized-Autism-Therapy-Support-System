...
const NotificationsPage = () => {
    ...
    const getIcon = (severity) => {
        switch (severity) {
            ...
            default: return <FiInfo className="text-emerald-500" size={18} />;
        }
    };
    ...
    const getBadgeStyle = (severity) => {
        switch (severity) {
            ...
            default: return 'bg-emerald-500/15 border border-emerald-500/30 text-emerald-500';
        }
    };
                    ...
                    <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            <button
                                ...
                                className="btn-secondary flex items-center gap-2 border-slate-600 hover:border-slate-500 hover:text-white"
                            >
                            ...
                            <div className="divide-y divide-white/[0.05]">
                                {notifications.map((n) => (
                                    <div
                                        ...
                                        className={`p-6 cursor-pointer flex flex-col sm:flex-row gap-5 transition-all group relative overflow-hidden ${n.isRead ? 'hover:bg-white/[0.02]' : 'bg-gradient-to-r from-emerald-500/[0.05] to-transparent hover:from-emerald-500/10'
                                            }`}
                                    >
                                        {!n.isRead && (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-2">
                                                <div>
                                                    <h4 className={`text-base leading-snug tracking-wide group-hover:text-emerald-400 transition-colors ${!n.isRead ? 'font-black text-slate-100' : 'font-bold text-slate-300'}`}>
                                                        {n.title}
                                                    </h4>
