...
const Therapy = () => {
    ...
    const fetchTherapyData = async () => {
        try {
            const [childRes, actRes] = await Promise.all([
                ...
                therapyApi.get(`/api/activities/recommend/${id}`)
            ]);
    ...
    const handleCompleteActivity = async (activityId) => {
        try {
            await therapyApi.post(`/api/activities/complete/${id}`, { activityId });
            fetchTherapyData();
        ...
        <div className="flex min-h-screen bg-app">
            ...
            <div className="flex-1 overflow-auto">
                <div className="max-w-7xl mx-auto p-8 page-enter">
                    {/* Header */}
                    <div className="mb-8">
                        <Link to={`${BASE}/parent/children/${id}`} className="text-teal-400 hover:text-teal-300 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest mb-6 transition-colors w-fit"><FiArrowLeft size={12} /> Back to Profile</Link>
