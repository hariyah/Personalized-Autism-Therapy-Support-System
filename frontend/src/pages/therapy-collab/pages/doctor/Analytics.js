...
import { getUrgencyChartData } from '../../utils/analysisInsights';
...
const Analytics = () => {
    ...
    const urgencyAnalyses = (analyticsData?.urgencyDistribution || []).flatMap((item) => Array.from({ length: item.count }, () => ({ urgencyLabel: item._id })));
    const urgencyChartData = getUrgencyChartData(urgencyAnalyses);
            ...
            <div className="flex-1 overflow-auto">
                <div className="max-w-7xl mx-auto p-8 page-enter">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="card p-8 border-subtle card-glow">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-lg font-bold text-slate-200 flex items-center gap-2">
                                    <FiActivity className="text-rose-400" /> Urgency Mix
                                </h2>
                                <span className="text-[10px] uppercase font-bold tracking-widest text-slate-500 px-3 py-1 bg-white/[0.05] rounded-md border border-white/[0.08]">
                                    Current load
                                </span>
                            </div>
                            <UrgencyDonutChart data={urgencyChartData} emptyMessage="No urgency data" />
                        </div>
                    </div>
                </div>
            </div>
