...
const PatientProfile = () => {
    ...
    const handleAnalysisUpdated = (updatedAnalysis) => {
        setAnalyses(prev => prev.map(item => item._id === updatedAnalysis._id ? updatedAnalysis : item));
    };
    ...
    if (!patient) {
        return <div className="flex min-h-screen bg-app"><Sidebar /><div className="flex-1 flex items-center justify-center flex-col text-slate-500"><FiUser size={32} className="mb-4 opacity-50" /><Link to={`${BASE}/doctor/dashboard`} className="text-violet-400 hover:text-violet-300">Patient not found. Back to Dashboard</Link></div></div>;
    }
                ...
                <div className="max-w-7xl mx-auto p-8 page-enter">
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                ...
                                <section>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {analyses.length > 3 && <button onClick={() => setActiveTab('analyses')} className="w-full py-4 text-xs font-bold text-slate-500 uppercase tracking-widest hover:text-violet-400 transition-colors">View All {analyses.length} Records â†’</button>}
                                    </div>
                                </section>
                    ...
                    {activeTab === 'analyses' && (
                        <div className="space-y-4 fade-in">
                            ...
                            {analyses.length === 0 ? <p className="text-slate-500">No records found.</p> : analyses.map(a => (
                                <div key={a._id} className="space-y-4">
                                    <AnalysisCard analysis={a} isDoctorView />
                                    <DoctorReviewPanel analysis={a} onSaved={handleAnalysisUpdated} />
                                </div>
                            ))}
                        </div>
                    )}
