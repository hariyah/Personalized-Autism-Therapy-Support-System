...
const PatientProfile = () => {
    ...
    const urgencyData = chartData.reduce((acc, item) => {
            ...
            <div className="flex-1 overflow-auto">
                <div className="max-w-7xl mx-auto p-8 page-enter">
                    ...
                    <div className="flex p-1.5 bg-white/[0.02] border border-white/[0.06] rounded-xl w-fit mb-8 shadow-inner">
                        {['overview', 'analyses', 'care-plan', 'chat'].map(t => (
                        ...
                        <div className="space-y-6 fade-in">
                            {analyses.length === 0 ? (
                                <div className="card p-10 border-dashed border-white/[0.1] text-center text-slate-500 text-sm font-medium">
                                    No analyses are available for review yet.
                                </div>
                            ) : (
                                <>
                                    <DoctorReviewPanel
                                        ...
                                        onSaved={(updatedAnalysis) => {
                                            setAnalyses((current) => current.map((item) => item._id === updatedAnalysis._id ? updatedAnalysis : item));
                                        }}
                                    />
                                    <CarePlanBoard
                                        analyses={analyses}
                                        isDoctorView
                                        onOpenReview={(analysis) => {
                                            setActiveTab('care-plan');
                                            setAnalyses((current) => {
                                                const next = current.filter((item) => item._id !== analysis._id);
                                                return [analysis, ...next];
                                            });
                                        }}
                                        onDownloadReport={(analysisId) => downloadAnalysisReport('doctor', analysisId)}
                                    />
                                </>
                            )}
                        </div>
