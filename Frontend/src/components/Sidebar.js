...
const Sidebar = () => {
    ...
    const navItems = isParent ? [
        { name: 'Dashboard', icon: FiGrid, path: '/parent/dashboard' },
        { name: 'My Children', icon: FiUsers, path: '/parent/children' },
        { name: 'New Analysis', icon: FiPlusCircle, path: '/parent/new-analysis' },
    ] : [
        { name: 'Dashboard', icon: FiGrid, path: '/doctor/dashboard' },
        { name: 'My Patients', icon: FiUsers, path: '/doctor/patients' },
        { name: 'New Analysis', icon: FiPlusCircle, path: '/doctor/new-analysis' },
        { name: 'Analytics', icon: FiTrendingUp, path: '/doctor/analytics' },
    ];
    ...
    return (
        <div className={`relative h-screen bg-[#0d1220] border-r border-white/[0.06] flex flex-col sticky top-0 z-40 transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'}`}>
            ...
            <div
                ...
                onClick={() => navigate(isParent ? '/parent/dashboard' : '/doctor/dashboard')}
            >
