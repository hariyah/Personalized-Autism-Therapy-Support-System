...
const AI_PREDICT_URL = process.env.AI_PREDICT_URL || 'http://localhost:8000/predict';
...
router.post('/recognize/:childId', upload.single('image'), async (req, res) => {
    try {
        ...
        const response = await axios.post(`${process.env.AI_SERVICE_URL}/predict`, formData);
