...
const populateAnalysisAccess = (query) => query
    ...
    .populate({
        ...
        populate: [
            ...
            { path: 'assignedDoctors', select: 'name' }
        ]
    });
...
router.put('/analyses/:analysisId/review', async (req, res) => {
    try {
        ...
        const isAssignedDoctor = analysis.child.assignedDoctors?.some(doctor => doctor._id?.toString?.() === req.user._id.toString());
        if (!isAssignedDoctor) {
...
router.get('/dashboard/analytics', async (req, res) => {
    try {
        ...
        const urgencyDistribution = await Analysis.aggregate([
            { $match: { child: { $in: patientIds } } },
