...
- Loads a lightweight base model (MobileNetV2 as used in train_quick)
- Unfreezes top layers for a small number of epochs with augmentation
- Saves a native Keras model to `models/densenet121_emotion_model.keras`

This is intended to be a short, CPU-friendly fine-tune (5-10 epochs).
"""
...
# Reuse helper from train_quick if available
try:
    from train_quick import find_image_directory, create_quick_model
except Exception:
    # Minimal fallback if train_quick isn't importable
    ...
    def create_quick_model(num_classes=6):
        # Use DenseNet121 to match the production inference model
        base_model = keras.applications.DenseNet121(
            weights='imagenet', include_top=False, input_shape=(224,224,3)
        )
        base_model.trainable = False
        inputs = keras.Input(shape=(224,224,3))
        # We will apply DenseNet preprocessing in the data pipeline, so pass inputs directly
...
IMG_SIZE = (224,224)
LEARNING_RATE = 1e-5
MODEL_SAVE_PATH = 'models/densenet121_emotion_model.keras'
CLASS_INDICES_PATH = 'models/class_indices.json'

# Allow overriding epochs via environment variable for quick tests
EPOCHS = int(os.environ.get('FINETUNE_EPOCHS', 10))
...
def main():
    ...
    # Face-cropping helper (works on numpy image arrays HxWx3, dtype=uint8)
    def crop_face_np(img_arr, target_size=IMG_SIZE):
        try:
            ...
            # Return image in 0..255 float32 (DenseNet preprocess_input expects this)
    ...
    # Data generators with augmentation; preprocessing_function applies face-crop + resize
    datagen = ImageDataGenerator(
        preprocessing_function=lambda x: tf.keras.applications.densenet.preprocess_input(
    ...
    # validation generator: deterministic face-crop/resize but no augmentation
    ...
    # Save class indices (inverse mapping to index->label)
    ...
    try:
        ...
        weights = compute_class_weight('balanced', classes=class_labels, y=classes)
        class_weight = {int(label): float(w) for label, w in zip(class_labels, weights)}
