...
- Model discovery order: `ML_MODEL_PATH` env > `ml_service/BEST_MODEL_PATH.txt` > latest matching file in `ml_service/models/` with prefix `densenet121_emotion_model*` and suffix `.keras` or `.h5`.
...
## Environment & Scripts
- Startup: `START_ALL_SERVICES.bat` sequentially (ML if model present, then backend, then frontend) + opens browser.
- ML launch (logged): `ml_service/start_service_with_env.bat` sets env thresholds; logs to `ml_service/logs/ml_service.log`.
- Training interactive: `RUN_TRAINING.bat` wraps `train_model.py`.
...
2. `BEST_MODEL_PATH.txt` has absolute path to chosen model.
